						  ...
						  out float4 outSMPosition  : POSITION1)
{
	...
	outSMPosition = mul(inPosition, lwvp);
}
						 ...
						 out float4 outDiffuse		: SV_Target )
{
	...
	float SMZValue = g_SMDepth.Sample(g_SMSamplerState, SMTexture).r + 0.0001f;
	...
	else if (SMZValue < ZValue && Material.Diffuse.a == 1) // ライトPOSから計算　SM＝ライトからの情報　Z＝視点からの情報
	{
		color.rgb = color.rgb * 0.5;   // 影の色
	}
	else
	{
