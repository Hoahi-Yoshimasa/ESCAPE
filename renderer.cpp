...
{
	return &g_LightViewBuffer;
}


ID3D11Buffer **GetLightProjection(void)
{
	return &g_LightProjectionBuffer;
}

ID3D11Buffer **GetWorld(void)
{
	return &g_WorldBuffer;
}
...
//	XMVECTOR pos = XMLoadFloat4(&g_Light.Position[0]);
//	XMVECTOR dir = XMLoadFloat4(&g_Light.Direction[0]);
...
//	mtxLightProjection = XMMatrixOrthographicLH(SCREEN_WIDTH, SCREEN_HEIGHT, VIEW_NEAR_Z, VIEW_FAR_Z);
...
void SetRenderer(void)
{
	...
	g_ImmediateContext->PSSetSamplers(1, 1, SMSampler());
	g_ImmediateContext->PSSetShaderResources(1, 1, SMResource());
