...
{
	float len;							// float型のlen という箱を用意する
	XMVECTOR a = XMVector3Length(*v1);	// ベクトルの長さを一旦VECTOR型で返す
	XMStoreFloat(&len, a);				// VECTOR型からfloat型に直す
	return len;							// float型になった長さをリターンする
}
...
BOOL RayCast(XMFLOAT3 xp0, XMFLOAT3 xp1, XMFLOAT3 xp2, XMFLOAT3 xpos0, XMFLOAT3 xpos1, XMFLOAT3 *hit, XMFLOAT3 *normal)
{
	...
	{	// ポリゴンの外積をとって法線を求める(この処理は対象が固定物なら予めInit()で行っておくと良い)
		vec1 = p1 - p0;
		vec2 = p2 - p0;
		crossProduct(&nor, &vec2, &vec1); // cross 外積
	...
	{	// 求めたポリゴンの法線と２つのベクトル（線分の両端とポリゴン上の任意の点）の内積とって衝突している可能性を調べる
		d1 = dotProduct(&vec1, &nor); // dot 内積  角度が求まる(鈍角-か鋭角+か)
